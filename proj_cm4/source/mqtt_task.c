...
void mqtt_client_task(void *pvParameters)
{
    ...
    /* VCM configuration for primary connectivity core (CM4) */
    cy_vcm_config_t primary_core_config;
    primary_core_config.hal_resource_opt = CY_VCM_USE_HAL_RESOURCE;
    primary_core_config.channel_num = CYHAL_IPC_CHAN_0;
    ...
    {
        printf("\nVCM Initialization failed on CM4.\r\n");
        CY_ASSERT(0);
    }
    else
    {
...
static cy_rslt_t mqtt_connect(void)
{
    ...
    for (uint32_t retry_count = 0; retry_count < MAX_MQTT_CONN_RETRIES; retry_count++)
    {
        ...
        if (CY_RSLT_SUCCESS == result)
        {
            result = cy_mqtt_get_handle(&mqtt_connection, MQTT_HANDLE_DESCRIPTOR);

            if (CY_RSLT_SUCCESS == result)
            {
               printf("MQTT handle descriptor '%s' created for CM0+\n", MQTT_HANDLE_DESCRIPTOR);
            }
...
static void vcm_callback(cy_vcm_event_t event)
{
    ...
    {
        printf("received INIT_COMPLETE from CM0+\r\n");
        init_done = 1;
        break;
    }
    case CY_VCM_EVENT_DEINIT:
    {
        printf("received DEINIT from CM0+\r\n");
        break;
    }
    default:
    {
